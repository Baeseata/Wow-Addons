...
第 4 步(右栏)已发,审查 16 条里**修了 11 条**,剩下这些是**代码 / 数据要动**的,不是"要确认一下":
...
- 🔴 **`258045`(毒牙 warglaive)的 tooltip 渲染的是裸物品,不是 311。**
  ...
  于是 `statless` 真,`offTrack` 跟着真,`DetailBonusID` 返回 nil。
  影响 3 个恶魔猎手专精(577 / 581 / 1480)。
  ⚠ **别在面板层加启发式绕过去** —— 15 件真·无副属性的遗产装备(1030 / 1041 的艾泽里特件)
  **每一件都有主属性**,而 `offTrack` 的真正含义是「这件东西没有当季升级轨道」,
  那是**来源**的属性,数据里现在没有这个字段。⇒ 修在生成器:要么解析出它的属性,要么显式标一个
  "on current track" 字段。**修之前先钉 build 跑一次 `gen_loot.py`**(理由见 CLAUDE.md)。
- **`test_gearrank.lua` 那节 statless guard 的 `SPECS = { 72, 258 }` 覆盖不到武器类**
  —— 狂暴战和暗牧都拿不了 warglaive,所以上面那条它**一条都看不见**。
  改成遍历全部 40 个专精(会当场报出 `258045`,所以要连上面那条一起修)。
- **`ns.SourceCandidates` 的护甲类型过滤没有任何断言**:把
  `keep = ARMOR_SUBCLASS[subclassID] == armorType` 改成 `keep = true`,
  两个测试**全绿**(实测暗牧在 1030 从 13 行涨到 34 行,21 件是皮/锁/板)。要补 guard。
- **`DetailBonusID` 的 344 分支从没被执行过**:`ceiling follows the card` 取的是
  `LootCardList("raid")[1]` = `b2849`,那张卡**一行 344 都没有**。
  要改成在 `b2883`(5 行)或 `b2895`(1 行)上断言,否则只验了 334 那一半。
- **harness 的银行是"从没打开过"那一种**(`GetContainerNumSlots` 只对 bag 0 返回非 0)
  ⇒ `ns.LootOwnedIndex` 的整个银行分支、`lootOwnedBank` 措辞、
  以及"装备 > 背包 > 银行"的优先级**一条都没跑过**。
- **本季唯一那根法杖(wand)对所有 40 个专精都不显示** —— `LOC.RANGED` 只挂在 RANGED 形状上。
  那个取舍是给**部位面板**定的(法杖要跟法杖竞争一个格子);来源列表回答的是
  「这个本掉不掉我能用的东西」,继承过来未必对。**要不要改是设计决定,先问 Jerry。**
