...
## 🟡 DodoInspect:三条「我们写并列、而攻略已经拆开」的属性数据(2026-08-28 assertion 9 暴露)

**为什么进这里**:交付物是 `Data/StatPriority.lua` 的数据订正,不是一份结论。
这批此前全被埋在 `same*` 里,是 assertion 9 把 `same*` 拆开之后才露出来的。

**是同一类,别当三例修**(现看:`cd DodoInspect && python tools/scan_statpriority.py --cached`):

| spec | 我们发的 | 当前攻略页 |
|---|---|---|
| **73 防战** `0\|raid` | `haste > crit=versatility > mastery` | `haste > crit > versatility > mastery`(Survivability 框) |
| **253 兽王** `43\|raid` | `mastery > crit=haste > versatility` | `mastery > crit > haste > versatility`(Pack Leader / All Situations) |
| **1473 增辉** `36\|raid` | `mastery > crit=haste > versatility` | `mastery > crit > haste > versatility`(Scalecommander) |

⇒ **共同形状:我们并列的那两个,攻略现在给了先后。** 先问一句「这个并列当初是**读来的**
还是**我们自己判的**」—— 是我们自己判的就得有理由,不能因为上游拆开了就跟着拆
(今天 260 那条正是反过来:两个源分歧,Jerry 拍板取一个,并保持 `provisional`)。
⚠ 73 那条还多一层:攻略那个序**正好等于我们自己的 `0|mythic` 行** ⇒ 要么我们的 raid 行错了,
要么这个专精 raid/M+ 本来就不该分。

⛔ **别动 258 暗牧那两条**(`archon` / `voidweaver` 报 `else?` / `bucket?`)——
那是 2026-08-28 **故意**跟 Wowhead 分开的(源是死链站,取 Icy Veins + Method),
理由写在 `DodoInspect/CLAUDE.md` 1.13.1 第三轮那节。**它会永远这么报,这是对的。**

🟢 **可做可不做**:268 酒仙两条 `tie-order` —— 分档跟攻略完全一致,只是并列组我们写
`crit=versatility=mastery`、攻略写 `versatility=crit=mastery`。**语义没差**,
跟着源改一下能让它读 `same`、少两行永久噪音;不改也不错。
