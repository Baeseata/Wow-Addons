...
## 🔴 DodoInspect:两个属性优先级扫描器的复核盲区(2026-08-28 回源那轮发现)

**为什么进这里**:它让**错的数据能安静地发出去** —— 这两个脚本的全部用处就是拦住那件事。
背景 / live 实例 / 为什么后果是「读数被污染」写在 `DodoInspect/CLAUDE.md`
的「同轮发现」那节,**别在这儿重读一遍**。这里只留要动的手。

### 1. `scan_statpriority.py` 从来没比过 `mythic` 那一半

`tools/scan_statpriority.py:453` 是唯一做精确比对的那行,bucket 写死成 `raid`;
`grep -c mythic tools/scan_statpriority.py` = **0**。要做三件:

1. **比对扩到 `mythic` bucket** —— guide 上的 `Mythic+` 框本来就解析出来了
   (270 织雾、1468 恩护各有一个),只是没人拿它去对我们的 mythic 行。
2. **收紧 `anywhere`** —— 现在它匹配**任意 tree + 任意 bucket**,一个 `same*` 同时表示
   「bucket 弄混了」和「天赋弄混了」。至少把这两种分开报。
3. **被断言 4 丢掉的 guide 框要变成显式的「未验证」计数**。现在末尾那句
   `rows differing from shipped N` 把 `DIFFERS` 和弱 `same*` 混在一个数里,
   而被丢掉的框(如 1480 的 void-scarred)**一个数都不出现**。

**别抄任何计数,跑这条现看**(纯本地,不花 API):

    cd DodoInspect && python tools/scan_statpriority.py --cached

🔴 **A/B 是硬要求**(canon:新增 / 收紧 guard 必须种一个真违规):先断言违规**真的种进去了**,
再看它变红且**指得准**;「改完还是绿的」不算验证。
现版本的负对照**已经跑过**:把全部 mythic 行打乱 → **0 失败**(它什么都不会发现)。
⇒ 改完这条同样的负对照必须变红。

### 2. (小)`scan_wcl_stats.py` 打印出来的 FLAG 定义,跟代码不是一回事

`BANNER` 写着 FLAG 的意思是 "we lead with X and out there X is last",还加了一句
**"that -- and nothing else -- is what a FLAG means here"**。而 `verdict()` 里有**三条**路
产出 `FLAG` / `FLAG?`,那句话只对应第一条(`top in firsts and obs[-1] in firsts`)——
它要求**观察到的第一名也得是我们排第一的**,所以它实际只能表达
「我们**并列**第一的那几个里,有一个在外面垫底」。

⇒ 只排了一个第一名的专精,那句话**结构上走不到这条路**:
49 个 spec+bucket 组合里只有 **9 组**的 `firsts` 有 2 个以上成员,其余 40 组永远进不去。
250 血DK 的 mplus 就是反例 —— `firsts = {versatility}`,
「我们排全能第一而外面全能垫底」完全就是 banner 那句话,却只能从 `top not in firsts`
那条路进去,而且**只要他们的宝石 / 附魔挑的也是全能,就打印 `loot?`**。

⇒ **二选一,别留着**:把 banner 改成如实描述那三条路,
或者把第一条路扩成「我们排第一的那个被观察到垫底」。
现在是**打印出来的定义比代码窄**,而读的人只看得到打印出来的那句。
(顺带:`FLAG?` 和 `no-data` 两个返回值在 `CLAUDE.md` 那张「判决只有四种」的表里没有。)
