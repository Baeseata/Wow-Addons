...
| **怎么点** | 三个入口:盘面 · 键位绑定 · **`/dodosays <象限>`**;面板一个按钮生成四个宏(`Dodo Cross/Square/Triangle/Circle`,图标 = 团队标记本尊 `137000+marker`) | 2026-08-16 群友提。**轮盘插件(OPie 一类)吃宏、不吃键位绑定** ⇒ 没有命令它们够不着我们；宏是唯一能上技能栏的东西。⛔ **ActionBook 集成不做** —— 宏对所有人有用(技能栏 + 任何吃宏的东西),ActionBook 只服务 OPie 用户还引入可选依赖,被完全盖住。⚠ 宏名的 `Dodo` 前缀是**安全要求**不是风格:`EditMacro` 撞名会**直接改写玩家自己的宏** |
