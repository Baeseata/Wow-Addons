...
⇒ 当时的修法是**补一节** `The real client: everything secret`，只在那一节里把 `issecretvalue` 照真机建模，
A/B 验过（拆掉围栏 → `got 8, want 6`，精确指出两次主阶段施法被误收）。

⚠ **2026-08-16 后记**：补一节挡不住**下一条新写的**测试 —— 它默认仍然落在明文那支。
所以那一节已被并入 **harness 的默认世界**（凡递给插件的值一律 secret，明文要 `H.plain(…)` 显式声明；
见 `CLAUDE.md` §2），**它作为独立小节不再存在**，别照上面那个旧名去文件里找。
本节其余数字全是 8/15 的原始实测，不动。
