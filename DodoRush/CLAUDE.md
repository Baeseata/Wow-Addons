...
- 0.1.0 实机验证 ✓(OMEN,Jerry 跑了多局):加载/开窗/横移/过门/接战/结算全链路工作。
- 0.1.0 实测死第 6 关 → 0.1.1 调参(数学分析详 Track.lua 参数块注释):死因 = 开局 α 0.50
  容错过窄 + 第 5 关 Boss 实际 α 0.72(门只按普通 α 补偿,基准都净损 21%)+ 第 6 关 BOTHBAD
  解锁三重叠加;且 need 第 11 关破 3.3 后全乘法门,落后玩家失去加法门追赶 => 第 8 关起数学必死。
  改:ALPHA0 0.50→0.38、GAIN 0.016→0.012、BOSS_MULT 1.28→1.18(CAP 0.92→0.90)、
  TRAP 3关18%→4关15%、BOTHBAD 6关12%→8关10%、SKIRM_FRAC 12~22%→10~18%。ρ 系列不动。
- **0.1.1 调参后 Jerry 实测到第 12 关,拍板:这条曲线就是"新手第一次玩"的基线,先不再调**。
- **大方向已定(2026-06-11 Jerry 拍板):做成养成式(meta-progression)** — 天赋点等
  局外持久成长让玩家走得更远,当前曲线 = 无加成下的新手基线。**具体玩法创意 Jerry 亲自构思中,
  Claude 不要自行设计**;等设计出来再动工。
- **全英文化(2026-06-11)**:6 个 .lua + .toc 的注释 + UI 字符串零中文 + 英文 README.md,
  与 DodoPool/DodoBricks 同标准(为未来 CurseForge 发布)。翻译只动注释/字符串,代码零改。
...
1. **【主线】养成式(meta-progression)设计 — 等 Jerry**:天赋点等局外成长,
   创意 Jerry 亲自构思中,**Claude 不要抢跑自行设计**。设计聊定后再排实现顺序。
   (难度曲线已锁定为无加成新手基线,养成数值将叠在它之上 — 届时大概率把 Track 的
   α/ρ/START_PAR 等参数变成"基础值 x 天赋修正"的形态,设计时记得这层。)
2. **英文化后回归一眼**(/reload 即可):开始界面/HUD/结算面板/暂停提示/小地图 tooltip
   文字显示正常、不超框(English 文案比中文长,重点看 HUD "Menu" 按钮和 hint 五行)。
3. **首跑未覆盖项确认**(0.1.0 已验证核心循环,这些边角是否都过了):
   散兵绕开判定手感、进战暂停 + 脱战缓起、ESC 关窗、"Continue Run"、结算面板纪录、
   音效响度听感(不满意换 KITS)。Lua 报错记下来修。
