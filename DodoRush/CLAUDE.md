...
**0.1.1(2026-06-11)— 实机已能跑通核心循环,第一轮难度调参完成,待复测手感**。
- 0.1.0 实机验证 ✓(OMEN,Jerry 跑了多局):加载/开窗/横移/过门/接战/结算全链路工作。
- **实测问题:难度曲线太陡,多局均死在第 6 关**(目标:熟练 20~30 关)。
- 0.1.1 调参(数学分析详 Track.lua 参数块注释):死因 = 开局 α 0.50 容错过窄 +
  第 5 关 Boss 实际 α 0.72(门只按普通 α 补偿,基准都净损 21%)+ 第 6 关 BOTHBAD 解锁三重叠加;
  且 need 第 11 关破 3.3 后全乘法门,落后玩家失去加法门追赶机制 => 第 8 关起数学必死。
  ...
  新曲线:加法门活到 ~28 关,落后 40% 玩家 22 关前可追赶,死亡区 28~35 关,Boss 净增
  ×1.07(5关)→×0.76(20关)→×0.48(30关) 渐进收紧。
...
1. **【最优先】0.1.1 难度复测**(/reload 即可):跑 5~10 局看死在第几关
   (目标:熟练 20~30 关;上一轮 0.1.0 死第 6 关,已调参,预期 15 关+)。
   重点感受:前 5 关是否爽(人数滚雪球的快感)、Boss 是否有惊无险、10+ 关压力是否渐显。
   太难/太简单继续按 §6 调 ALPHA0/GAIN(主杠杆);门数字观感(NiceNum 取整)是否舒服。
2. **首跑未覆盖项确认**(0.1.0 已验证核心循环,这些边角是否都过了):
   散兵绕开判定手感、进战暂停 + 脱战缓起、ESC 关窗、"继续本局"、结算面板纪录、
