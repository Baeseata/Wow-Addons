...
6. 🔴 **武器两行整个没上过真机**(2026-08-14 第三轮写完,已拷进游戏目录未 reload)。
   ⚠ 离线测试到 **128 checks**,但那证明的是排序和过滤,**UI 一行都没跑过** ——
   切换按钮、面板高度封顶、截断标题、副手的三种空状态文案,全是纸面正确
...
### 2026-08-14 OMEN 第三轮:武器两行(**已写完,未发版**)
...
**还欠着的**:
- **排序口径未拍板** —— 武器行仍是「属性契合 + 344 档提升」。是否改成**装等档优先**(理由:
  武器伤害由装等定,而装等现在读得准)是设计决策,归 Jerry。**截断能不能接受取决于这条。**
- **类型角标列没做** —— 切换让主手在每个模式内同质了,但**萨满的副手是盾+副手物品混排**,
  这一列重新有意义。
- 真机验收(装进游戏了,Jerry 未 reload)。
