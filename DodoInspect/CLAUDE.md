...
#### 🔴 触发场景已确认:**团本战斗(灭团时)** —— 而这条规则 canon 两周前就写下了

Jerry 2026-08-30 回报:**在团本里,灭团的时候**。⇒ 不是战场 / 竞技场,
**是这个插件用户的主场景**。(「灭团」那半句别当机制读 —— 它更可能只是他那会儿正好在点人;
必要条件是「团本副本里 + 有一次 inspect 落地」。)

🔴 **canon [`rules/wow-addons.md`](~/.claude/canon/rules/wow-addons.md) 的「三条硬关门」里写着**
(2026-08-16,**DodoSays 实测**,毒瀑深渊):

> **`UnitGUID` 在这种受限地图里也是 secret** ⇒ 凡「拿 GUID 认身份」的写法全废。
> ⚠ 顺带:**存进 state 的那个 secret 值本身是 truthy** …… **读到 secret 就当 `nil` 存。**

**「读到 secret 就当 nil 存」逐字就是这次的修法** —— 它比这次崩溃早了两周,
而且**落地实现就在同一个仓里**:`DodoSays/Detector.lua` 的 `guidOf()`。
同期 DodoInspect 自己的 doc 里写的却是「目标的 GUID 可读」。
⇒ **失效点不是「没人知道这条规则」,是「没人意识到此刻适用」** ——
两个插件隔了两周、隔了一个文件夹,一个学到了,另一个把相反的前提写进了文档。
**这正是本条配 guard 而不是配一段散文的理由。**

⚠ **DodoSays 那份还多做了一件我们没做的事,别抄漏**:它的 `isOurBoss()` 是**三个分支**
(是 / 不是 / **问不出来 → 回落到别的判据**),因为 2026-08-15 它写成 `== true` 时
`nil == true` 折成了 `false`,**整场零报点、零报错**。我们这边比较处读不到就**不过滤直接 Render**,
同样是「第三支不当 false」,但形状简单得多(我们只有一个判据,没有可回落的第二个)。

#### ⚠ 同一个根因的**安静版本**:团本里英雄天赋那行不会显示(而这是对的,别去修)

`InspectPanel.lua:376` 的 `if unitGUID and unitGUID == inspectReadyGUID` 在团本里
**永远不成立** —— `ReadableUnitGUID` 老实答 nil。于是 `ns.InspectHeroSubTree()` 不被调用,
面板上那行英雄天赋在团本里一直是空的。**这在修之前就已经这样**,不是这次改出来的。

⛔ **别把这个闸拆掉当「修好了」。** `ns.InspectHeroSubTree()` 读的是**全局检视缓存**
(`C_Traits` 的 `INSPECT = -1`),里面装的是**最近一次被检视的那个人**,不保证是面板在显示的人。
GUID 那一比是唯一确认「这份数据属于这个面板」的东西 ⇒ 拆了它 = 把**别人的**英雄天赋
画到这个人的面板上,**而且永远看不出错**(第五颗雷那句「标签的主体和数字的主体必须是同一个」)。
**现在是 fail-closed,少一行信息 —— 那是这两个结局里对的那个。**

🧪 **想救回来需要一次真机探针,不是一次重构**:INSPECT_READY 的事件参数**是明文的**
(这次报错的 locals 就是证据),所以「这份数据属于谁」我们知道;缺的是「面板在显示谁」。
⇒ 团本里量一下还有哪个身份信号明文(`UnitName`/`UnitFullName` 对**队友**、
`UnitIsUnit("target", "raidN")`、`UnitGUID` 对 `player` 自己),有明文的就能配一条第二判据。
**没量到之前别动**,`DodoProbe` 就是干这个的。
...
小地图图标 → 一个自由浮动窗口:左栏切「大秘境 / 团本」并列出卡片(8 个副本 / 9 个 boss),
点卡片 → 右栏出该项对当前选中专精的掉落。
...
🔴 **2026-08-24 Jerry 换了挂载方式:不挂 `ChallengesFrame`,改小地图按钮 + 自由窗口。**
理由是他那句「之前本打算的侧栏会被 raid io 占用」—— RaiderIO 已经占着那块地方,
两个插件抢同一个锚点,报回来的样子会是「装了别的插件我们就叠上去了」。
**别照设计稿开头那段原始需求写码** —— 它整段被同文件「2026-08-24 改版」那节取代;
原文故意留着,因为它解释了「固定高度 / 放不下加滚动条」这些约束当初从哪来的,
而那些约束换成自由窗口之后**不再成立**。
...
### 2026-08-24:第 3 步落地(小地图按钮 + 窗口壳 + 左栏)

进度以 `git log` 为准;这里只留不会漂的那几条。文件 = `Minimap.lua`(按钮)/
`LootPanel.lua`(窗口)/ `ns.LootCardList`(卡片列表,住 **`LootSource.lua`**)。

- **卡片列表故意不住 `LootPanel.lua`** —— 它是对 `Data/Loot.lua` 的纯数据整形,零 frame,
  所以 `tools/test_gearrank.lua` 能直接 `loadAddonFile` 它。放进 UI 文件就再也测不到了。
- 🔴 **「团本 9 个 boss」这个 9 是从 `Data/Loot.lua` 数出来的,含义是「有掉落的 boss」**,
  不是「团里的 boss」。本季恰好两个团本(`1320` 八个 + `1317` 一个)。哪天某个 boss 掉的
  东西不在表里,它就**没有卡片** —— 这是设计,不是缺陷,测试里有一条断言钉着它。
- 🔴 **小地图图标的 texcoord `0.16` 是量出来的,不是抄 LibDBIcon 的 `0.07/0.93`。**
  他们那组数字是用来裁**暴雪图标自带的边框**;`Media/Dodo.tga` 没有边框,它有 ~20% 的
  **透明留白**(实测不透明像素 x 15–50 / y 11–52),照抄会把鸟画成隔壁按钮的一半大。
  重画那个 tga ⇒ 重量一次(量法:读 tga alpha 通道的包围盒)。
- **本机小地图角度**:8 个 Dodo 按钮全挤在 200°–265°(`grep -rn 'minimapAngle|minimap = { angle' Wow-Addons`),
  所以这个用 **295**。⚠ 只对 Jerry 自己那台机有意义 —— CF 上的用户没有别的 Dodo 插件。
- **进 CF 包的入口只有小地图按钮**,而它可以在设置里关掉 ⇒ **`/dins loot` 必须一直能开**。
  别把 slash 那条当「顺手加的」删掉,它是关掉按钮之后唯一的路。

#### 🔴 harness 教训:stub frame 的 `Create*` 返回 self,会把窗口自己关掉

写 `tools/test_lootpanel.lua`(驱动 `SetupLootPanel` / `RefreshLootPanel` 的离线冒烟)时
第一版 stub 用了一个 catch-all `__index`,任何方法都返回 `self`。于是:

- `panel.title = panel:CreateFontString(...)` ⇒ **`panel.title` 就是 `panel` 本身**;
  `panel.empty` / `panel.divider` 同理。
- `RefreshLootPanel` 里那句 `panel.empty:Hide()` ⇒ **把整个窗口 Hide 了**,
  于是 harness 报「`Show()` 没生效」。**bug 完全在 harness 里,被测代码是对的。**
- 同一个 catch-all 还让 `panel.cards = panel.cards or {}` 永远拿到那个函数
  (真客户端里 `panel.cards` 是 `nil`)⇒ 下一行 index 它就炸。

⇒ **stub 要照生产现实建模:没设过的字段就是 `nil`**,只有**方法形状的名字**(大写开头,
WoW 的命名约定)才答函数,而 `Create*` 一族必须**返回新对象**。
(canon `rules/engineering.md`:夹具的默认值必须指向生产现实。)

⚠ **`tools/test_lootpanel.lua` 证不了「画得对不对」** —— stub 会开开心心接受一张画到
窗口外面去的卡片。它只证明这条路跑得通;几何和可读性仍然只能真机看。
它跟 `tools/test_gearrank.lua` 分开是因为后者**停在数据层**:卡片列表对不对有断言,
而「列表怎么被画出来」此前一条断言都没有(canon guard 家族 (i):两套测试各自完备、
中间那段接缝没人覆盖)。


#### ⚠ 这一步真机第一次跑要看的(离线一条都验不到)

前面那两个测试证明的是**数据对**和**这条路跑得通**;下面每一条都只有真客户端答得了。
按风险排:

1. **小地图按钮画出来了吗、是不是一坨白**。空白 = `Media/Dodo.tga` 那条路径没解析
   (SetTexture 失败是**静默**的);代码里有问号兜底,所以真出现的应该是问号不是空白 ——
   **看到空白说明连兜底都没走到**,那是另一回事。
2. ~~**卡片有没有图标**~~ ✅ **2026-08-25 实测:8 张全有图,不需要 lazy load**
   (`GetMapUIInfo` 的 texture 是引擎侧给的)。以下原文留着,因为它记着当初那三条
   互相矛盾的类推长什么样:原设计说「要」、改版时我写「不要」、本机三个在产插件
   都不加载它 —— 而那三个**没有一个取第 4/5 个返回值**,也就是没有一个碰过 texture。
   **三条全是类推,零条是读数。**
3. **团本那 9 张卡片的名字**。冷登录时 `EJ_GetEncounterInfo` 先答 nil,靠有限次重试补上 ——
   等 3 秒还是空的,说明重试次数不够或者根本没触发。
4. **窗口高度**:它取两种模式里较高的那个,所以**切一下模式窗口不该跳**。跳 = `Layout()`
   里那个 `tallest` 没算到某一边。
5. **拖动 + 关掉 + 重开,位置记住没有**;`/reload` 之后还记不记得。
6. **Esc 关得掉吗**(`UISpecialFrames`)。
7. **字号滑块拖一下**,卡片列表不该爬到 tab 上面去(`ApplyChrome` 就是干这个的)。
### 2026-08-25:第 4 步落地(右栏 = 掉落列表)

进度以 `git log` 为准;这里只留不会漂的。数据层 = `ns.SourceCandidates`(**住 `GearRank.lua`**,
挨着 `SlotCandidates`)+ `ns.SpecWeaponLocations` + `ns.HasSecondaries`;持有扫描 =
`ns.LootOwnedIndex`(**住 `LootSource.lua`**);画的部分在 `LootPanel.lua`。
`GearPanel.lua` 导出了三个共用件:`ns.GearTooltipLink` / `ns.DedupeByName` / `ns.SetStatCells`。

- 🔴 **tooltip 的档位跟着**卡片**走,不跟着面板走。** 设计稿和 `Config.lua` 里
  「M+ 用 `GEAR_HERO_BONUS_ID`(311)」那句是 **8-24 加团本那半之前**写的 ——
  照它一刀切,团本那 9 张卡片的掉落会被**低报 23~33 级**,正好是它警告的那个错误的镜像。
  ⇒ `DetailBonusID`:`dungeon` → 311;`boss` → 照 `GearPanel` 原来那套 334 / 344。
  guard 里那条 `ceiling follows the card` 就是钉这个的,A/B 验过。
- 🔴 **盾牌和副手拿的东西是 `classID == 4`(护甲),但它们归**武器形状**管,不归熟练度管。**
  第一版拿 `ns.WeaponProficient` 判它们 —— 那问的是 **CAN**,而 `WEAPON_SHAPE` 问的才是 **SHOULD**,
  两者恰好在这儿分家:**每个圣骑士都 CAN 拿盾**,于是**双手惩戒骑和狂暴战都被塞了盾**。
  ⚠ **这个 bug 不是任何断言发现的**,是把「每个专精实际拿到哪些 equipLoc」打印出来一眼看见的
  (`spec 70 TWOHAND -> INVTYPE_2HWEAPON INVTYPE_SHIELD`)。
  ⇒ 修法 = 盾 / 副手一律走 `ns.SpecWeaponLocations` 那个并集;guard 补了**两个方向**
  (「不许多给」+「不许少给」),少一个方向就漏。
- **`statless` 现在跟 `score` 分开算**(`ns.HasSecondaries` 单独一个谓词)。
  `statless` 问的是**这件东西**有没有副属性,`score` 问的是**这个专精**有没有属性优先级去给它打分 ——
  以前两者永远同时成立,所以没人分得出来。分开之后:**没有属性优先级的专精照样看得到掉落**,
  只是不排序、不编号(第三个返回值 `ranked` 就是干这个的)。
  ⚠ 这不是洁癖:**40 个专精里有 8 个的优先级按英雄天赋分**,拿不到英雄树就 resolve 不出来 ——
  第 5 步一旦允许选**别人**的专精,那 8 个当场撞上,而 `SlotCandidates` 在那种情况下返回 nil。
- **`simRank` 带着但绝不参与排序。** 饰品模拟给的是**序数**(1、2、3),`StatFit` 给的是 **0..1 的分数**;
  部位面板上一次只有一种在场,所以那边排得了。这边一个 boss 同时掉饰品和胸甲,
  拿序数跟分数比大小 = 饰品永远第一,而理由只是「排名从 1 开始」。
- **持有状态只扫实物**(装备栏 → 背包 → 银行,取最强的那个),装等从**实物 link** 读。
  设计稿里「已拥有-勇士 / 英雄」做不成的证据别再查一遍;顺带一提,**扫实物那 39 件没外观的
  饰品/戒指/项链照样准**,这才是选它的真正理由。银行没开过 = 读不到 = **什么都不显示**,
  **绝不显示「你没有」** —— 这一列只回答「在哪儿」,从不回答「你没有」。

#### 🔴 harness 教训(第二条):`tools/test_lootpanel.lua` 之前**根本够不到右栏**

第 3 步那份 harness 里 `_G.C_Item = {}` 且**从没 stub 过 `GetItemInfoInstant`** ——
`GearRank` 的 `ItemShape` 拿不到函数就**静默返回 nil**,于是 `SourceCandidates` 返回**空表**;
再加上没有专精 API,`PlayerSpecID()` 答 nil,右栏**连列表都走不到**就去画那句提示了。
⇒ **先写断言、后加 stub**,A/B 是这么跑的:三条右栏断言在加 stub 之前**全红**
(`the right column drew no rows at all`),加完才绿。**「加了断言还是绿的」在这儿等于零信息量。**

顺带把 stub 补成更像真的:`CreateFrame(_, name)` 现在会**把有名字的框体注册进 `_G`**(真的就是这么干的),
`SetText` / `GetText` 真的存值 —— 后者是必须的,否则 `panel.detailTitle:GetText()` 拿回来的是**框体自己**,
`框体 .. " "` 当场炸。⚠ 顺手把生产代码里那次 `GetText()` 回读也去掉了:
那个字符串 `RefreshDetail` 自己手上就有,绕一趟控件只是给自己找一个会骗人的依赖。


#### 2026-08-25 真机后追加:部位列 + 饰品排最前(Jerry 当场提的)

- **右栏现在是五列**:名字 / **部位** / 大绿字 / 小绿字 / 持有。
  部位那格走 **`ns.L.slots[equipLoc]`** —— **一个新字符串都没加**:那张缩写表
  (`TR` / `饰` / `BI` / `AB`)四语早就齐了,背包格子上的部位角标用的就是它。
- 🔴 **饰品整块排在最前面。** 这是**反着**属性排序的:33/42 的饰品没有副属性 ⇒
  `StatFit` 给 nil ⇒ 它们本来沉在最底下,而那恰恰是开一张 boss 卡最想看的东西。
- 🎁 **顺带把 `simRank` 救回来了。** 之前它被带着但**不参与排序**,理由是「序数 vs 0..1 的
  分数不可比」—— 那个理由**只在混排时成立**。饰品自成一块之后,块内每一行都在同一个
  序数标尺上 ⇒ 模拟排序在块内合法。**同一个字段,换个结构就从有害变成有用。**
- ⚠ **别照抄 `SlotCandidates` 那句 `if trinketRow and not trinketRank then return {}, 0`**:
  40 个专精里有 23 个没有饰品模拟数据,照抄会让这些专精**一个饰品都看不到**,
  而 boss 照样掉。这里的做法是照列不误,只是块内不排序。
- 🔴 **比较函数里那句 `if aTrinket and bTrinket`,两个都要判。**
  只判 `a` 今天也是对的(上面那条分支已经保证同类),但那是**隔着距离成立的不变式** ——
  A/B 时把「饰品优先」那行去掉,`table.sort` 当场抛 `invalid order function`
  而不是排错序,于是**数据层那条 guard 一条都没跑到**,只有 UI 那条报了。
  ⚠ 这也是一条给 A/B 脚本的教训:**崩溃也是红**,而它既不以 `FAIL` 开头也不含 `ERROR`
  —— 我的分类器差点把它读成「这条 guard 只被 UI 测试抓到」。

#### ⚠ 第 4 步真机要看的(离线一条都验不到)

1. **右栏画出来没、四列对不对齐**;字号滑块拖大拖小,名字那列该截断而不是压到属性列上。
2. **卡片切换右栏跟着换**,而**窗口高度不该跳**(高度只由左栏和 `MIN_DETAIL_ROWS` 决定)。
3. **hover 一行看 tooltip 的装等**:大秘境卡片该是 **311**,团本卡片该是 **334**
   (毒渊最后两个 boss 的**带主属性**装备是 **344**,戒指项链仍是 334)。**这是这一步最值钱的一眼。**
4. **标题右边那个 `上限 311`** —— 它是**从 tooltip 用的同一条 link 读回来的**,不是写死的常量。
   要是显示成别的数,说明 `GEAR_HERO_BONUS_ID` 该重新推了(换赛季必然)。
5. **持有那一列**:身上穿着的、包里有的、银行里的各找一件对一下;**没有的那些必须是空白**。
   ⚠ 银行没开过就读不到,那时候整列空是**预期**。
6. **`N/M`** 只在装不下时才出现 —— 本季实测最多 14 行,应该看不到它。

### 2026-08-27:第 5 步落地(职业 + 专精两级下拉)

进度以 `git log` 为准;这里只留不会漂的。数据层 = `ns.LootSpecIndex`(**住 `LootSource.lua`**,
挨着 `LootCardList`,零 frame 所以 `test_gearrank` 也够得到);选择器 + 画的部分在 `LootPanel.lua`
(`ns.LootPanelSpec` / `ns.SetLootPanelSpec` / `ns.LootPanelClass` / `ns.SelectLootClass`)。

- 🔴 **存的是「偏离」,不是「当前专精」。** `DodoInspectDB.lootPanelSpec` **缺席 = 跟随当前角色**;
  只有在看**别人**的专精时才落盘。反过来存(把当前专精写进存档)的症状极毒:换个小号登录,
  窗口给你上一个角色的列表,而**代码完全正确、界面完全正常** —— canon 那条「代码是对的、坏的是状态」。
  ⇒ 配套:**点自己的专精 = 清掉那个键**(不是存自己),否则以后重新加点会被冻在旧专精上。
- 🔴 **英雄天赋只在「看自己」时才传下去。** `PlayerHeroSubTree()` 读的是**你当前专精**的树,
  拿它去给别人的专精排序 = 用你的天赋排他的装备,而且**永远看不出错**。
  ⇒ 判据就一个 `if isOwn`;测试里必须**把 `C_ClassTalents` 也 stub 上**,否则两边都答 nil、这条断言白给。
- **排不了序要说出来。** 40 个专精里那 **8 个**(实测 = `62 65 102 103 250 253 1473 1480`)的属性优先级
  按英雄树分,选别人的当场就撞上 ⇒ `ranked = false` ⇒ 不编号。光不编号读起来像「编号坏了」,
  所以标题尾巴上挂一句 `ns.L.lootUnranked`。**这不是把它当 bug 修**(设计稿明说别修),是把设计说出口。
- 🔴 **专精名单的来源是 `ns.SpecGear`,不是客户端的专精表。** 方向反了会提供
  `SourceCandidates` 答不了的专精,而唯一的反馈是一个空右栏 —— 读起来像「这个 boss 不掉你的东西」。
  从自己的表出发 ⇒ **那个状态结构上到不了**。客户端只被问它独有的两件事:这专精归哪个职业、这俩叫什么。
- **同职业内的顺序 = specID 升序**,而且**别拿一张手写顺序表去修它**。暴雪基本按显示顺序发的 id,
  唯一对不上的是武僧(酒/风/织雾 → 酒/织雾/风)。为一个换位在派生表旁边放一张手写表 = 又一份会漂的事实。
  真要修就从客户端的枚举里取顺序。

#### 🔴 契约查了、但**只查了一个文件** —— 差点把两个在产 API 判成不存在

`GetSpecializationInfoForClassID` / `GetSpecializationInfoByID` 在生成契约
`SpecializationInfoDocumentation.lua` 里**一个都没有** —— 那份只覆盖 `C_SpecializationInfo` 命名空间,
这两个是全局。**而 Plater 在同一个客户端上拿它们给 13 个职业建缓存**(`Plater.lua:6493`)。
⇒ canon `rules/engineering-evidence.md` 那条「说某平台做不到 X 之前,问一句这个能力**如果存在**会住在哪几个文件里」
的原样复刻。**证据的缺席 != 缺席的证据。**

⇒ 落地写法:**命名空间优先、裸全局兜底、两边都 `type(...) == "function"` 守**;
主路径只用**契约里查得到的** `C_SpecializationInfo.GetClassIDFromSpecID` +
`C_CreatureInfo.GetClassInfo`(两个都有在产插件在用),所以**那两个全局在不在都不阻塞**。

#### 🔴 harness 教训(第三条):stub 不真跑 `SetupMenu`,整个菜单闭包一行都没执行

`stubFrame` 的 catch-all 会把 `SetupMenu` 吞成「返回自己」的空函数 ⇒ 生成器**从来没被调用**,
而 25 步全绿。**加了断言还是绿的,在这儿又一次等于零信息量。**
⇒ `CreateFrame` 现在按 `frameType` 分家:`DropdownButton` 拿一份**真的**
`SetupMenu`(存)+ `GenerateMenu`(跑,并把 `CreateRadio` 的条目记下来),
测试于是能**照点击那条路**触发 radio 的回调,而不是绕过 UI 直接调 `ns.SelectLootClass`。

⚠ **同一轮还抓到一条真空断言**:原本写的是「调两次 `LootSpecIndex`,比较两次的顺序」——
而它 **complete 之后就缓存**,第二次拿回的是**同一张表**,等于拿自己跟自己比。
改成断言**单调性**(按名字升序 + id 兜底),这才是真的在看东西。

#### ⚠ 第 5 步真机要看的(离线一条都验不到)

1. **两个下拉画出来没、点得开没**,里面是不是 13 个职业 / 当前职业的 3~4 个专精。
   空白或点不开 = `WowStyle1DropdownTemplate` 那条 `pcall` 失败了(那时窗口会**照常工作**、只是没有下拉)。
2. 🔴 **`GenerateMenu()` 每次 refresh 都跑** —— 确认它**不会自己把菜单弹开**。
   抄的是 DodoGuanzhu 的现成序列(`SetupMenu` → `SetDefaultText` → `GenerateMenu`),
   但那是**设置面板**,这是个**开着的窗口 + 每次刷新都调**,这一处是新的。
3. **字号滑块拖到最大 / 最小**:下拉该跟两个模式按钮在同一条带子上,不该压到卡片或右栏标题。
   最小档 `TAB_H` 现在有下限(`DROP_MIN_H + 4`),所以**最小档窗口会比以前高一点点,这是预期**。
4. **标题会不会被挤掉**:团本卡片的标题本来就长,现在最多再多两截(`上限 334` + `无属性优先级`)。
   `SetWordWrap(false)` ⇒ 超了是**截断**不是换行。截掉了就把那句挪个地方。
5. **选另一个职业 → 右栏真的换了**;选**自己的职业** → 弹回**自己当前那个专精**(不是该职业第一个)。
6. **选一个「按英雄树分」的专精**(如 `62` 奥法 / `250` 鲜血)→ 行**不编号** + 标题挂 `无属性优先级`;
   换一个不分树的(如 `63` 火法)→ 编号回来、那句消失。
7. **拖动 + `/reload`**:专精选择该记住;**换个小号登录 → 自动跟着新角色走**(不是记住上一个号的)。
8. **重新加点**(自己的专精变了)→ 窗口跟着变(`PLAYER_SPECIALIZATION_CHANGED` 已注册)。
...
### 🔑 换季 / 调优后怎么复核 —— **现在是一条命令,别再手写 curl 循环**(2026-08-27 重写)
...
跟在产数据的 `same / same* / DIFFERS`,末尾按 `dateModified` 聚类。
配套 `tools/dump_statpriority.lua`(用真 Lua 加载器把在产数据打平,diff 的是**表**不是人的印象)。
⚠ **脚本头部那 5 条断言一条都别删** —— 每条对应下面一个实测过的坑,而**每个坑的失败方向都是静默的**。
(⚠ 这里原写「8 条」,`grep -nE '^  [0-9]+\. ' tools/scan_statpriority.py` 数出来是 **5** 条,
它自己的头也写着 `FIVE ASSERTIONS`。**别照数字去删补**,以脚本头部为准。)

**原来这节写的 curl SOP 有三处是错的**(2026-08-27 逐条实测):

1. 🔴 **「顺序 = 抓 `[ol]…[/ol]`」返回空列表。** BBCode 躺在 JSON 串里,闭合标签**全部转义**成
   `[\/ol]` / `[\/li]`。实测增强萨那页:`[ol]` 2 个、`[/ol]` **0 个**、`[\/ol]` 2 个 ⇒
   那条正则**一页都匹配不到**,而**空列表读起来就是「这页没有顺序」**。
   ⇒ 解析前先 `html.replace(chr(92) + "/", "/")`。
2. 🔴 **UA 写得越像真浏览器越会被 403。** 同一个 URL 同一秒实测:
   `-A "Mozilla/5.0"` → 200(69,488 字节)· `-A "curl/8.0"` → 200 · **不带 UA** → 200 ·
   `-A "Mozilla/5.0 (Windows NT 10.0; … Chrome/120 …)"` → **403 Request blocked**(CloudFront)。
   ⇒ **就用 `Mozilla/5.0`,别「改进」它。**
3. 🔴 **「分不分内容 = `[tab name=`」已经不成立** —— `[tab ` 在**当前 40 页里出现 0 次**。
   内容分格现在一律是**并排的两个框**:`[box border=true]` 或 `[div style="border:…"]`,
   框头一个 `[center]` 标题。⚠ **这正是 1468 恩护那次栽的坑的一般形式**,现在有判据了 ——
   **标题就是答案**,它长这样:
   - `[symbol=wow-hero-talent-aldrachi-reaver] [b]Aldrachi Reaver[/b] Stat Priority` → **分英雄树**
   - `[b][color=q9]Survivability[/color] Stat Priority` / `DPS Stat Priority` → **分内容**(坦克)
   - `[b] Preservation [color=venthyr]Raid[/color] Stat Priority` / `Mythic+` → **分内容**
   - 框内还能再套一层 `[b]Single-Target:[/b]` / `[b]AoE:[/b]`(猎人页就是)
   ⇒ **两个并排的框先读标题,别数框。**

**另外四条是这次新撞出来的:**

4. 🔴 **英雄树不能按「离 `[ol]` 最近的 `wow-hero-talent-*`」判。** 实测:暗牧那页第二个列表前
   95 字符处躺着一个 `wow-hero-talent-**totemic**`(暗牧没有图腾),德鲁伊野性那页把
   `druid-of-the-claw` 重复在了 Wildstalker 的位置 —— **两个都产出一个结构完整、读着毫无问题的错答案**。
   ⇒ 先用 DB2 取**这个专精自己的**树集合,不在集合里的符号**只统计不使用**;树名优先从**框标题**认。
5. **`[li]` 有第四种写法** `[li icon=angle-right]`(实测一页 31 个 `[li]` vs 41 个闭合)。
6. **列表里合法出现的非副属性**:`Item Level` / `Weapon Damage` / `Armor` / `Item Level (Agility+Stamina)`
   / `Item Level / Agility / Armor / Stamina`。还有 `Crit` 缩写 · `Haste/Crit` 用斜杠表示并列 ·
   `Mastery to 1200 rating` 把目标写成尾巴。⇒ 忽略名单必须**显式列举**,
   千万别写成「映射不上就跳过」—— 那等于把**每一种未来的新写法**都变成一条静默变短的顺序。
7. **我们的 schema 表达不了的形式确实存在**:噬灭DH 的 Void-Scarred 是
   `Haste(到 800) > Crit > Mastery > Vers > Haste(超过 800)` —— **急速出现两次**。
   「恰好四个副属性各一次」那条断言正是在这儿开火的,**它不是误报,是如实说「这条塞不进来」**。

🔴 **subTreeID 绝对不许猜 —— 而且 DB2 本身有个陷阱。** 增强萨在 `TraitTreeLoadout` 里映射到
**三棵** trait tree(786 / 1033 / 1034),每棵都带一整套 Stormbringer/Totemic/Farseer,
**名字、描述、图标 atlas 全部一模一样**(54/55/56 · 70/71/72 · 73/74/75)。
其它 12 个职业都只映射到一棵 ⇒ **没有任何东西会警告你**。
判据只有一个:**`SkillLineXTraitTree` 里那个职业 skill line 指向的那棵**(萨满 skillLine 924 → 786),
1033/1034 **在那张表里根本不存在**。同一个方法复现了已在游戏内验证过的 DK(750)和圣骑(790)。
⇒ 增强萨 = **Totemic 54 / Stormbringer 55**;`scan_statpriority.py` 现在自动带出这个号。
⚠ 写错的后果是**静默**:`ResolveDefault` 是 `if subTreeID and specData.builds[subTreeID]`,
配上 `if not specData.raid then return nil end` ⇒ 号错 = 那个专精两棵树**全部失去属性优先级**,
零报错、形状 guard 也抓不到(它只查形状不查号)。

**时机比方法重要**(2026-08-27 又量了一遍,结论更硬):**40 页里 35 页的 `dateModified` 挤在
`2026-08-12 22:54:39–49` 这 11 秒内** = 一次 Wowhead 批量重发布,**不是 35 个作者各自改了**。
真正动过的只有 5 页:1468(08-13)· 252(08-17)· 261(08-19)· 263(08-22)· 581(08-25)。
⇒ **先看聚类再决定这轮值不值得做**;落在那个批量时间戳上的 `DIFFERS` 一律**不是上游改了**,
而是「我们当初就选了别的」或者「解析器读错了」—— 那是另一件事,别混进数据刷新里。
附:Icy Veins 正文能 curl 到(逐页 `Updated: <日期>`,有分辨力),但**很多专精跟 Wowhead 同一个作者**,
拿它当第二源要先看署名 —— 同一个人写的两份**不构成独立确认**。
Method.gg 正文是 JS 渲染的,curl 只拿得到日期,正文要走 WebFetch。

### 2026-08-27 第二轮复核:263 拆树 · 581 三并列(**改的是数据,发版状态照本文规矩现查**)

上面那次(08-22)动了 3 个专精;这次是**同一条工作线**的第二轮,又动了 2 个。
两条的性质**又不一样**,别当成一批:

| spec | 改动 | 性质 |
|---|---|---|
| **263 增强萨** | 1×1 → **2 树 × 1**:Totemic(54)`M=H>C>V` **不动**;Stormbringer(55)`C=M>H>V` | 作者 08-22 23:48 真改了 |
| **581 复仇DH** | `H > C=V > M` → **`H > C=V=M`**(三并列)+ 标 `provisional` | 作者**加了一张图表**,而正文说三者约等 |

🔴 **581 那条值得记 —— 它是「源更新了」的一种新坏法**:作者 08-25 新增了一张
`H > C > V > M` 的图表,**而同一页的正文一个字没改**,仍写着几条副属性
"roughly equal, closer to the difference between .95 and 1 in value rather than .1 and 1"、
全能 "equally as good as Critical Strike"、精通 "slightly better than Crit outside of Meta"。
⇒ **照图表原样落地 = 声称一个作者本人明确否认的精度。** 我们发的是**两者都支持的那个更弱的主张**:
只有急速领先,其余三个并列。Icy Veins(08-26)把它们排成 `H > M > V > C` ——
**两个源分别把精通放在第二和最后** ⇒ 挂 `provisional`。
🔑 **可迁移:一个源内部「图表 vs 正文」打架时,取交集不取图表。** 图表看着更权威
(它有结构、能直接抄进代码),而正文才是作者说话的地方 —— **好抄不等于更真**。

⚠ **263 的 subTreeID(54/55)是查出来的不是猜的**,而 DB2 在这儿有个专门坑人的形状 ——
见上一节末尾「subTreeID 绝对不许猜」那段,别重新查一遍。

### 🔬 第二条信道:`tools/scan_wcl_stats.py`(WarcraftLogs 上排名玩家**实际穿的**装备)

上面那个扫的是**攻略作者怎么说**;这个扫的是**排行榜前列的人实际穿了什么**,
然后跟 `Data/StatPriority.lua` 逐专精 diff。可行性调研 + 成本实测在
[`STAT_PRIORITY_WCL_RESEARCH_2026-08-27.md`](STAT_PRIORITY_WCL_RESEARCH_2026-08-27.md)。

```bash
python tools/scan_wcl_stats.py --plan     # 只报开销,一点都不花
python tools/scan_wcl_stats.py            # 全量抓 + 出表(680 次调用 / ~1374 点 / 38% 时额度)
python tools/scan_wcl_stats.py --cached    # 复用上次抓的,改解析器时用
python tools/scan_wcl_stats.py --only 581  # 单个专精
python tools/scan_wcl_stats.py --verify    # 9 条负对照,跑完就退
```
凭据 = `C:\Users\Doodo\.private\wcl-api.json`(仓库外),`WCL_API_JSON` 可覆盖;
**明文永不打印,只打长度**。token 实测 `expires_in` ≈ 360 天。
缓存 `tools/.scan-cache/`(**262 MB,已进 `.gitignore`**)。

🔴 **它的输出只许当反证,不许当正证** —— 这是**断言 7**,写死在脚本里:
判决列**故意没有**「他们跟我们一致所以我们对」这个值。人穿什么是
**攻略(同一批我们也在读的)+ 掉落运气 + 装等压倒副属性**的合成结果。
「我们写精通第一而外面精通垫底」值得回头查;「他们精通高」**不证明**精通更好。

**判决只有四种**(2026-08-27 全量 80 行:`ok` 52 · `thin` 11 · `loot?` 7 · **`FLAG` 7**,另 3 行 `n<20` 不判):

| verdict | 意思 |
|---|---|
| `ok` | 跟我们一致 |
| `thin` | 领先幅度 < `MIN_MARGIN`(3 个百分点)⇒ 读不出信号 |
| `loot?` | 穿的跟我们不一致,**但他们自己镶的宝石 / 附魔跟我们一致** ⇒ 掉落供给,不是偏好 |
| **`FLAG`** | 穿的和挑的**都**不一致 ⇒ **只有这一档值得回头查** |

🔑 **这套两道闸是本轮最值钱的东西,别退回去只比排名。** 第一版只比「谁第一」,
80 行报了 **25 条**;逐条量领先幅度才发现 **20 条不到 5 个百分点、11 条不到 3 个、一条只有 0.2**,
而且 **13 条里他们主动挑的东西跟我们一致** ⇒ 那 25 条量的是**掉落池的偏斜**,不是玩家偏好。
加闸后 25 条干净地裂成 **7 + 11 + 7**(相加正好 25 = 天然 A/B)。
**宝石和附魔之所以是第二条信道,是因为它们不受供给约束** —— 想镶哪个镶哪个。
⚠ `loot?` **不是「弱一点的 FLAG」**:它是关于世界的**另一个事实**(这专精想要的东西掉得少),
合并进 FLAG 就把两个都读没了。

#### 🔴 脚本头部那 9 条断言一条都别删 —— **每条的失败方向都是静默的**

正文在 `tools/scan_wcl_stats.py` 的模块 docstring 里(带实测数),这里只留索引,
好让你知道**碰哪一块该回去读哪一条**:

| # | 一句话 | 不守它会怎样 |
|---|---|---|
| 1 | `className`/`specName` 要 **slug** 不是显示名 | 名字写错**不报错**,回一个空页 —— 读起来就是「这专精没人打」 |
| 2 | `gear[]` 里**每个数字都是字符串** | 按字符串排序,零报错 |
| 3 | `gear[]` 恒 18 格,空格子是**占位对象**不是 null | 按数组长度过滤 ⇒ 永远过滤不掉 |
| 4 | 🔴 **bonusID 会改「有哪两条副属性」** | 静默丢掉 **55,418** 件(见下) |
| 5 | `StatPercentEditor` 是**预算点**不是百分比,且**逐部位不同**(护甲 ~7000 / 戒指项链 ~17500) | 拿常数归一 ⇒ 戒指项链权重翻倍 |
| 6 | 治疗必须 `metric:hps` | `metric:dps` **照样回 100 条**,只是换了一百个人 |
| 7 | 只许当反证(见上) | —— |
| 8 | zone id **钉死 + 跟线上核对** | 换季静默扫上个赛季 |
| 9 | 「第一名跟我们不同」本身**不算分歧**(两道闸) | 25 条待查里 18 条是噪音 |

🔴 **断言 4 单独说 —— 它是这次真正的工作量,也是最毒的那个坑。**
`ItemSparse` 里一批物品把副属性写成**哨兵** `24`/`25`(3500+3500 的通配槽,基础行**没有属性身份**);
真身份由**穿在身上那一件**的 `ItemBonus` `Type=25` 行带来,六个 bonusID 覆盖全部六种「四选二」:
`8790` 暴+急 · `8791` 暴+精 · `8792` 急+全 · `8793` 急+精 · `8794` 精+全 · `8795` 暴+全。
- **只读基础行 = 这些装备被判成「没有副属性」丢掉,而报表照样长得完整。**
  全量负对照:**带 bonusID 解出 520,946 件,不带 465,528 件 —— 差 55,418 件**。
- 🔑 **「物品 id → 属性」根本不是一个函数**:实测 **89 个** id 在不同角色身上解出**不同的属性对**
  ⇒ 缓存键必须是「id + bonusIDs」。⚠ 这跟 `gen_loot.py` 的世界观**不一样** ——
  那边一个 id 一行副属性,因为它描述的是**掉落表**(还没落到具体某个人身上)。
  **同一张 `ItemSparse`,两个问题,两个答案。**

**负对照 = `--verify`,9 条,其中 4 条是载重的**(它们**拆掉被测机制看结果变不变**,
不是「跑一遍看绿不绿」):把 `wild_map` 换成 `{}` 重算(上面那两个数就是它的读数)·
同一个 id 是否真解出多种 · `MIN_MARGIN` 归零后同一行是否从 `thin` 翻成 `FLAG` ·
去 slug 化的专精名是否**真的**被 gameData 拒绝。
⚠ 没有缓存时那条通配对照会 **SKIP**,脚本会明说「**SKIP 不是 PASS**」—— 别读成绿。
**2026-08-28 复跑:9/9 PASS。**

⚠ **样本量按专精差很多,报结论必须带 `n`**:同一个 boss 上惩戒骑 / 神牧 100+,增强萨 58,
**冰法只有 13**。`n < 20` 的行**一个判决都不出**(`MIN_N`)。
