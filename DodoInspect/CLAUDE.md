...
- **更新时间** = `"dateModified":"…"`。⚠ **34/37 页会挤在同一分钟**(Wowhead 批量重发布,
  ...
  可以拿来验这个量还有没有分辨力。
- **顺序** = 抓 `[ol]…[/ol]`。⚠ **三种写法都要认**(这里原本写的是「两种」,2026-08-24 撞出第三种):
  ① `[li][b]Haste[/b] (~700)` ② 裸的 `[li]Strength[/li]`(武器战,第一版 0 命中)
  ③ 🔴 **一个 `[li]` 里用 `=` 塞两个属性** —— `[li][b]Critical Strike = Mastery[/b][/li]`
  (增强萨 08-22 改版后就是这个)。**第三种的失败方向最毒**:它不是 0 命中,是**少命中** ——
  那一页照样返回一个结构完整、读起来毫无问题的顺序,只是**少了两个属性**
  (实测吐出 `H > V`,而真值是 `C = M > H > V`)。⇒ **凡提取出来的 order 不是恰好四个副属性,
  当场报错,别往下走**。`tools/test_statpriority.lua` 里那条「恰好四个各一次」的
  形状 guard 正好能当这个用 —— 但前提是数据先进了文件,而这一步在那之前。
