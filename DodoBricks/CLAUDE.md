...
核心玩法至此完整;剩余皆为可选润色(尾迹/危险警告/统计/0.3 新道具,见 §5)。选音听感一直没专门确认,默认 OK。
...
1. ~~0.2.0 实机验证~~ **已通过**。核心玩法完整,以下全是可选项(已向用户列菜单):
2. **润色候选**:
   - **球尾迹**(参考图同款彗星拖尾;注意球可上百颗,做好降级/池化);
   - **底行危险警告**(最底行砖呼吸闪红,提示快被压死);
   - **开始界面统计**(累计局数/累计碎砖/单局最高球数等,SavedVariables 加字段);
   - **0.3 玩法扩展**: 新道具(斜激光/全屏雷击?)、特殊砖(碎后掉奖励的宝箱砖?)——做前先和用户讨论设计。
3. **选音实机听感确认**(从未专门问过): hit/launch 轻嗒(856/857)高频下烦不烦?碎砖 MAP_PING、
