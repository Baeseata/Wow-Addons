...
- **球速渐变(0.2.2,用户主动提出)**: 基础球速随关数 +2%/关(上限 ×1.6,约 31 关到顶);
  单回合飞行超 4s 后再平滑快进(smoothstep,12s 拉满 ×2.0),总封顶 ×2.6。
  实现 = 纯"时间快进"(Driver FLY 分支统一缩放 dt,发射间隔/物理/滑行同步),**弹道与 1 倍速完全一致**;
  HUD 顶条"速 ×N.N"指示(×1.1 起显示)。早期回合短基本无感,后期省时间。
...
- 用户明确**不做**: 手动加速 / 提前回收**按钮**(0.2.0 用户再次拍板"暂时不需要";防水平死弹保留)。
  注意区分: 0.2.2 的**自动**球速渐变是用户主动要的,与"不要手动按钮"不冲突。
...
用户拍板: 危险警告/统计暂时不做,0.3 玩法扩展等用户想好再谈。

**0.2.2 已写完,⚠️ 未实机验证(2026-06-10)**: **球速渐变**(用户原话"越往后越快,后期不耽误时间",
当日还反馈游戏"解压又好玩")。关数基础提速 ×(1+0.02·(关-1)) 封顶 1.6 + 回合飞行 4s 起 smoothstep
快进至 ×2.0、总封顶 ×2.6;HUD"速 ×N.N"指示。实现 = Driver FLY 分支统一乘 dt(纯快进,弹道不变);
Physics.Step 原有 MAX_DT=0.05 钳制 → 低帧率 + 高倍速时快进自动饱和,不穿模、不死亡螺旋,百球时
每帧物理工作量也因此有上界。尾迹按帧记历史,快进时节距自然拉长 = 视觉更"快",属预期效果。
...
1. **0.2.1 尾迹 + 0.2.2 球速渐变实机验证**(/reload 即可): ① 拖尾好不好看、跟参考图比够不够味
   (嫌短/淡调 Render.lua `GHOST_SIZE/GHOST_ALPHA` 或 Game.lua `TRAIL_MAX/TRAIL_BUDGET`);球上百颗时帧率。
   ② 球速渐变: HUD"速 ×N.N"该出现时出现(第 6 关起 / 回合拖长时上涨、回合结束回落)、长回合是否明显省时、
   快进后有无穿砖(理论不会)、嫌快/嫌慢按 §6 调参(快进太猛先降 `RAMP_MAX`/`SPEED_MULT_CAP`)。
2. **暂缓项(用户拍板"暂时不用")**: 底行危险警告、开始界面统计。
3. **0.3 玩法扩展**: 新道具(斜激光/全屏雷击?)、特殊砖(宝箱砖?)——**等用户想好再谈设计**。
4. **选音实机听感确认**(从未专门问过): hit/launch 轻嗒(856/857)高频下烦不烦?碎砖 MAP_PING、
   激光 838、炸弹 5274 爽不爽?不行按 DodoPool 迭代路径换 `KITS`(终极方案 media/ 自带 ogg)。
...
**球速渐变**: `LV_SPEED_GAIN=0.02 / LV_SPEED_MAX=1.6` 关数基础提速 · `RAMP_START=4 / RAMP_FULL=12 /
RAMP_MAX=2.0` 回合内快进(几秒起步/几秒拉满/上限)· `SPEED_MULT_CAP=2.6` 总封顶
(`LV_SPEED_GAIN=0` + `RAMP_MAX=1` 即整体关闭)。
