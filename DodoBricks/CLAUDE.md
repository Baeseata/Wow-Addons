...
- 用户明确**先不做**: 加速按钮 / 提前回收按钮(物理层的防水平死弹保留)。后续道具(激光/炸弹)留润色。
...
**0.1.0 全部写完,⚠️ 尚未实机跑过一次(连加载报错都没验)。** 实现清单:
...
1. **【最优先】实机首跑**: 完全重启魔兽 → 看加载是否报错(逐个修)→ `/bricks` 开局:
   - 三角砖四个朝向画得对不对(SetVertexOffset 折叠是本插件最大的"理论正确但没见过"项)?
   - 瞄准虚线方向/反弹预览对不对?松开能发射?右键能取消?
   - 球弹砖/弹三角斜边/弹墙手感;砖扣血变色、碎砖;吃 +1;首球落点标记;下压动画 + 裁剪;
   - 压到底游戏结束、再来一局、自动存档(/reload 后"继续上次进度")、最高纪录、进战暂停。
2. **手感调参**(见 §6): 球速 / 发射间隔 / 出砖概率 / 三角占比 / 下压动画时长。
3. **选音实机验**: hit/launch 用的 UI 轻嗒(856/857)在高频下会不会烦?碎砖 MAP_PING 够不够爽?
   不行就按 DodoPool 的迭代路径换 `KITS`(终极方案 media/ 自带 ogg)。
4. **润色池(未开始,按用户兴致)**: 球尾迹、碎砖粒子/闪光、连击音调递增、清屏奖励、
   激光/炸弹道具、加速与提前回收按钮(用户说过先不做,想要了再加)、瞄准辅助档位。
