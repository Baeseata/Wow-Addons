...
1. ~~实机首跑~~ **已通过**(见 §4)。
2. **0.2.0 候选(已向用户列菜单,等拍板)**:
   - **加速按钮 ×1/×2/×3**(飞行中可点;0.1 砍掉的,后期球多回合变长后会是刚需)+ **提前回收**;
   - **碎砖反馈**: 白闪 + pop 缩放(无粒子系统,纯 texture 动画,便宜的爽感);
   - **清屏奖励**: 一回合打空全场 → 奖励球 + 飘字;
   - **新道具**: 横/竖激光(清行/列)、炸弹(3×3)——玩法深度,可放 0.3;
   - **手感调参**按用户实玩反馈(见 §6)。
3. **选音实机听感确认**: hit/launch 的 UI 轻嗒(856/857)高频下烦不烦?碎砖 MAP_PING 爽不爽?
   不行按 DodoPool 迭代路径换 `KITS`(终极方案 media/ 自带 ogg)。
4. **润色池(按用户兴致)**: 球尾迹、连击音调递增、瞄准辅助档位。
